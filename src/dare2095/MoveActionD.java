 ...
 * @author amy
 */
public class MoveActionD extends AbstractAction {
	 ...
	 */
	public static double TARGET_REACHED_ACCEL = 40;//0.0001;

	/**
	 * allowable error in the target error before you say it is there
	 */
	public static double TARGET_REACHED_ERROR = 50.0;//2.0;
	 ...
	 */
	public MoveActionD(Toroidal2DPhysics space, Position currentLocation, 
			Position targetLocation, Vector2D targetVelocity, AbstractObject target) {
		super();
...
//		KpTranslational = 0.0;
	 ...
	 */
	public MoveActionD(Toroidal2DPhysics space, Position currentLocation, 
			Position targetLocation, AbstractObject target) {
		super();
...
//		float Kp = 0.16f;
	...
	public boolean isMovementFinished(Toroidal2DPhysics space) {
		if(isFinished){
			...
			if(target != null){
				AbstractObject finalTarget = space.getObjectById(target.getId());
